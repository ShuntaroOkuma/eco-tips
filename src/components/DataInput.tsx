...
export default function DataInput({ onDataSubmit }: Props) {
  ...
  const parseInput = (text: string): ParsedData | null => {
    try {
      ...
      for (const line of lines) {
        ...
        if (line.includes('水') || line.includes('L') || line.includes('リットル')) {
          const match = line.match(/\d+(\.\d+)?/);
          if (match) data.water = parseFloat(match[0]);
        }
        if (line.includes('気温') || line.includes('温度') || line.includes('℃')) {
          const match = line.match(/\d+(\.\d+)?/);
      ...
      if (!data.electricity || !data.water || !data.temperature || 
          !data.humidity || !data.weather || !data.people) {
  ...
  const handleSubmit = () => {
    ...
    if (data) {
      ...
      setError('データの形式が正しくありません。以下の情報を含めてください：\n- 電力使用量（kWh）\n- 水使用量（L）\n- 気温（℃）\n- 湿度（%）\n- 天気\n- 在室人数');
    }
  };
      ...
      {parsedData && (
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <p>電力使用量: {parsedData.electricity}kWh</p>
            <p>水使用量: {parsedData.water}L</p>
